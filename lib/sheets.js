...
export function dataBlockRange(sheetName) {
  return `${sheetName}!A${DATA_START_ROW}:T`;
}
...
function isDataRowEmpty(row) {
  ...
  return (c === '' && g === '') || aToKEmpty;
}

/** 8行目以降の最初の空行。なければ末尾の次行（累積追加） */
export function findFirstEmptyRowInBlock(blockValues) {
  for (let i = 0; i < blockValues.length; i++) {
    if (isDataRowEmpty(blockValues[i] || [])) {
      return DATA_START_ROW + i;
    }
  }
  return DATA_START_ROW + blockValues.length;
}

export async function ensureSheetRowCapacity(sheetName, targetRow) {
  const { sheets, spreadsheetId } = getSheetsClient();
  const res = await sheets.spreadsheets.get({ spreadsheetId, fields: 'sheets.properties' });
  const sheet = (res.data.sheets || []).find((s) => s.properties?.title === sheetName);
...
export async function prepareNewDataRow(sheetName, targetRow) {
