...
export function normalizeSubstrate(value) {
  ...
  if (!text) return '';
  if (/^非\s*铁$/u.test(text) || /^非\s*鉄$/u.test(text) || /^非\s*鐵$/u.test(text) || /non.?ferrous/i.test(text)) return '非鐵';
  if (/^铁$/u.test(text) || /^鉄$/u.test(text) || /^鐵$/u.test(text) || /^Fe$/i.test(text) || /ferrous|steel/i.test(text)) return '鐵';
  return text;
}
