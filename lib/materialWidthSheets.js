...
import { buildPlistRowsFromEofficeItems } from './plistEofficeImport.js';
...
export async function analyzeMaterialWidthSheets() {
  ...
  const eofficeOnlyItems = eofficeOnly.map((code) => ({
    code,
    name: eofficeByCode.get(code)?.name || '',
    width: eofficeByCode.get(code)?.width || '',
  }));
...
export async function addEofficeOnlyItemsToPlist({ dryRun = true } = {}) {
  const { sheets } = getMaterialWidthSheetsClient([
    'https://www.googleapis.com/auth/spreadsheets',
  ]);
  ...
  const plist = await loadPlistWithMaterialWidth(sheets);
  const eoffice = await loadEofficeMaterialWidth(sheets);
  const { eofficeByCode } = buildMaterialWidthRecommendations({
    plistRows: plist.rows,
    eofficeRows: eoffice.rows,
    monthlyRows: [],
  });
  ...
  const plistCodes = new Set(plist.rows.map((r) => cell(r, 0)).filter(Boolean));
  const items = [...eofficeByCode.entries()]
    .filter(([code]) => !plistCodes.has(code))
    .map(([code, { name, width }]) => ({ code, name, width }));

  const built = buildPlistRowsFromEofficeItems(items);
  const rows = built.map((entry) => entry.row);
  const completeCount = built.filter((entry) => entry.meta.complete).length;
  const incomplete = built.filter((entry) => !entry.meta.complete);

  const result = {
    dryRun,
    eofficeSheet: eoffice.sheetName,
    plistBefore: plist.rows.length,
    addCount: rows.length,
    completeCount,
    incompleteCount: incomplete.length,
    incompleteSample: incomplete.slice(0, 20).map((entry) => ({
      code: entry.code,
      name: entry.meta.englishName,
      type: entry.meta.type,
      thickness: entry.meta.thickness,
      width: entry.meta.width,
      height: entry.meta.height,
      length: entry.meta.length,
      materialWidth: entry.meta.materialWidth,
    })),
  ...
  await sheets.spreadsheets.values.append({
    spreadsheetId: PQFORM_SPREADSHEET_ID,
    range: `${quoteSheetName(PLIST_SHEET_NAME)}!A:I`,
  ...
  return {
    ...result,
    plistAfter: plist.rows.length + rows.length,
    applied: true,
  };
}
