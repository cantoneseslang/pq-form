...
export const COL = {
  ...
  weight: 4,        // E 単重 kg
  rolls: 5,         // F 卷數
  totalKg: 6,       // G 總數 kg  ← 在庫計算に使用
  totalPrice: 7,    // H 總價
};

/** 在庫 API は A–H 列のみ読む。AP–AY は参照しない。 */
export const INVENTORY_LAST_COL = 'H';
...
function availableKgFromRow(row) {
  return parseNumber(cell(row, COL.totalKg));
}
...
export function parseInventoryRow({
  ...
  const availableKg = availableKgFromRow(row);
...
export async function loadRawMaterialTab(sheets, spreadsheetId, tabTitle) {
  ...
  return {
    ...
    inventorySource: 'A-H columns: F=卷數, G=總數kg',
