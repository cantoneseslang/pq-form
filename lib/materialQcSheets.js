...
export function getMaterialQcDriveClient() {
  ...
  if (!delegatedUser) {
    throw new Error(
      'GOOGLE_DRIVE_DELEGATED_USER 未設定。請在 Vercel 設定 Workspace 使用者電郵（例: you@kirii.com.hk）。',
    );
  }
...
export async function uploadQcPhotoToDrive({ imageBase64, mimeType = 'image/jpeg', receiptDate, lotNo }) {
  ...
  try {
    ...
    throw new Error(
      `Google Drive 上傳失敗 (${msg})。`
      + `資料夾: ${folderId}、使用者: ${delegatedUser}。`
      + '請確認 Google Admin 已啟用 Domain-wide Delegation，且該使用者可存取此資料夾。',
    );
  }
...
async function uploadQcPhoto({ imageBase64, mimeType, receiptDate, lotNo }) {
  if (!RAW_MATERIAL_QC_DRIVE_FOLDER_ID) {
    throw new Error('RAW_MATERIAL_QC_DRIVE_FOLDER_ID 未設定。');
  }
  return uploadQcPhotoToDrive({ imageBase64, mimeType, receiptDate, lotNo });
}
