...
export async function uploadQcPhotoToDrive({ imageBase64, mimeType = 'image/jpeg', receiptDate, lotNo }) {
  ...
  try {
    ...
    const hint = delegatedUser
      ...
      : `請使用共享雲端硬碟（Shared Drive）資料夾，並將 ${serviceAccountEmail} 設為「內容管理員」或以上。個人「我的雲端硬碟」資料夾無法使用。`;
...
async function uploadQcPhoto({ imageBase64, mimeType, receiptDate, lotNo }) {
  if (!RAW_MATERIAL_QC_DRIVE_FOLDER_ID) {
    throw new Error('RAW_MATERIAL_QC_DRIVE_FOLDER_ID 未設定。請在 Vercel 設定共享雲端硬碟資料夾 ID。');
  }
  ...
  try {
    ...
    try {
      ...
      throw new Error(
        `相片上傳失敗。Drive: ${driveError.message} / Supabase: ${supabaseError.message}`,
      );
    }
  }
}
