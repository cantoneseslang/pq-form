import { readRange, writeRanges } from './dailyReportSheets.js';
import { resolveDailyReportMonthFromIso, resolveDailyReportYearFromIso } from './dailyReportSheetMap.js';
...
const ALL_BLOCK_PATTERNS = [
  ...Object.values(MOLDING_MACHINE_PATTERNS),
  AUTO_MACHINE_PATTERN,
];
...
function isOtherMachineHeader(row, ownPattern) {
  return [0, 1, 2].some((col) => {
    const val = cellValue(row, col);
    if (!val || ownPattern.test(val)) return false;
    return ALL_BLOCK_PATTERNS.some((pat) => pat.test(val));
  });
}
...
function findBlockByPattern(rows, pattern) {
  ...
  for (let i = startIndex; i < rows.length; i++) {
    const row = rows[i] || [];
    if (i > startIndex && isOtherMachineHeader(row, pattern)) break;
...
function isSlotEmpty(row) {
  ...
  const fEmpty = !f || f === '0' || f === '0.0';
  const bIsMachineLabel = b && ALL_BLOCK_PATTERNS.some((pat) => pat.test(b));
  return !a && (!b || bIsMachineLabel) && !d && fEmpty;
}

export function isDailyReportRowEmpty(row) {
  return isSlotEmpty(row || []);
}

export const MOLDING_MACHINE_NAMES = Object.keys(MOLDING_MACHINE_PATTERNS);
...
/** 同一製品は既存行を更新。異なる製品はブロック内の最終データ行の次へ順次追加。 */
export function pickTargetRow(rows, block, productLabel) {
  ...
  let lastUsedIdx = -1;
  for (let i = 0; i < block.slotRows.length; i++) {
    const row = rows[block.slotRows[i] - 1] || [];
    if (!isSlotEmpty(row)) lastUsedIdx = i;
  }

  const nextIdx = lastUsedIdx + 1;
  if (nextIdx < block.slotRows.length) return block.slotRows[nextIdx];
...
function buildDailyReportUpdates(tabName, targetRow, values, block, pageType, machine) {
  const isFirstSlot = block && targetRow === block.slotRows[0];
  ...
  const updates = [];
  const dateLabel = formatDailySheetDate(values.__date);
  if (dateLabel) {
...
export async function clearDailyReportRow(tabName, targetRow, month, year) {
  if (!tabName || !targetRow) return { tabName, row: targetRow, cleared: false };
  const columnMap = ['A', 'B', 'D', 'F', 'H', 'I', 'J', 'K', 'L'];
  const updates = columnMap.map((col) => ({
    range: `${tabName}!${col}${targetRow}`,
    values: [''],
  }));
  await writeRanges(updates, month, year);
  return { tabName, row: targetRow, cleared: true };
}

async function resolveDailyReportContext(payload) {
  const {
    date,
    pageType = 'molding',
    machine,
    ...
    mainLines = [],
    materialLines = [],
    targetRow: explicitRow = null,
    tabName: explicitTab = null,
  } = payload;

  const tabName = explicitTab || dailySheetTabName(date);
  if (!tabName) throw new Error('invalid date for daily report tab');
  const sheetMonth = resolveDailyReportMonthFromIso(date);
  const sheetYear = resolveDailyReportYearFromIso(date);

  const values = buildDailyReportRowValues(mainLines, materialLines, productTypes);
  values.__date = date;

  const sheetValues = await readRange(`${tabName}!A1:L100`, sheetMonth, sheetYear);
  ...
  const targetRow = explicitRow || pickTargetRow(sheetValues, block, values.B);
  if (!targetRow) throw new Error('daily report machine block is full');

  return { tabName, targetRow, values, block, pageType, machine, sheetMonth, sheetYear };
}

export async function writeDailyReportEntry(payload) {
  const ctx = await resolveDailyReportContext(payload);
  const updates = buildDailyReportUpdates(
    ctx.tabName,
    ctx.targetRow,
    ctx.values,
    ctx.block,
  ...
  await writeRanges(updates, ctx.sheetMonth, ctx.sheetYear);
  const { __date, ...values } = ctx.values;
  return {
    tabName: ctx.tabName,
    row: ctx.targetRow,
    values,
    range: `${ctx.tabName}!A${ctx.targetRow}:L${ctx.targetRow}`,
  };
}

export async function updateDailyReportEntry(payload) {
  return writeDailyReportEntry(payload);
}

export async function deleteDailyReportEntry(payload) {
  const {
    date,
    pageType = 'molding',
    machine,
  ...
  let targetRow = explicitRow;
  if (!targetRow) {
    const productLabel = buildDailyReportRowValues(mainLines, materialLines, productTypes).B;
    const sheetValues = await readRange(`${tabName}!A1:L100`, sheetMonth, sheetYear);
    const block = pageType === 'auto'
      ? resolveAutoMachineBlock(sheetValues)
      : resolveMoldingMachineBlock(sheetValues, machine);
    if (block && productLabel) {
      for (const rowNum of block.slotRows) {
        const row = sheetValues[rowNum - 1] || [];
        if (productMatchesRow(row, productLabel)) {
          targetRow = rowNum;
          break;
        }
      }
  ...
  if (!targetRow) {
    return { tabName, row: null, cleared: false, skipped: true };
  }

  return clearDailyReportRow(tabName, targetRow, sheetMonth, sheetYear);
}
