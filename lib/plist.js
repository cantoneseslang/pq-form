...
export function thicknessMatchesPlist(requestedT, plistT) {
  const reqBase = thicknessForProductLookup(requestedT);
  const plistBase = thicknessForProductLookup(plistT);
  if (reqBase && plistBase && reqBase === plistBase) return true;
...
export async function searchPlist({ type, t, w, h, l, other = '' }) {
  ...
  const nt = thicknessForProductLookup(t);
...
/** plist 照合: 0.8A→0.8、0.4D→0.4、1.5A→1.5 など A/D サフィックスを除去して照合 */
export function thicknessForProductLookup(value) {
  ...
  if (m) return formatThicknessNum(m[1]);
  return formatted;
}
...
export const MANUAL_THICKNESS_OPTIONS = ['0.4D', '0.8A', '1.0A', '1.5A'];
