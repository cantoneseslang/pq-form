...
export function packMaterialData(material, materialLines, sheetRows, dailyReport = null) {
  ...
  delete summary.importedFromDailyReport;
  ...
  const importedFromDailyReport = summary.importedFromDailyReport === true;
  return {
    ...summary,
    lines,
...
export function unpackMaterialData(materialData) {
  ...
  const { lines, sheetRows, dailyReport, importedFromDailyReport, ...summary } = materialData;
...
export function dbRowToClient(row) {
  ...
  const { material, materialLines, sheetRows, dailyReport, importedFromDailyReport } = unpackMaterialData(row.material_data || {});
  return {
    id: row.id,
...
export function clientToDbInsert(record) {
  ...
  const materialForPack = {
    ...
    ...(record.importedFromDailyReport ? { importedFromDailyReport: true } : {}),
  };
  return {
