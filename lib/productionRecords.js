...
export function packMaterialData(material, materialLines, sheetRows) {
...
export function unpackMaterialData(materialData) {
  ...
  const { lines, sheetRows, ...summary } = materialData;
...
export function dbRowToClient(row) {
  ...
  const { material, materialLines, sheetRows } = unpackMaterialData(row.material_data || {});
  return {
    id: row.id,
...
export function clientToDbInsert(record) {
  ...
  return {
    ...
    material_data: packMaterialData(record.material || materialLines[0] || {}, materialLines, sheetRows),
