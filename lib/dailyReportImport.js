...
export async function repairImportedProductNamesForTab(tabName, supabase) {
  const { recordDateIso, entries, rows: sheetRows } = await scanDailyReportTab(tabName);
  if (!recordDateIso) {
  ...
  for (const row of dbRows || []) {
    ...
    const productInfo = await resolveProductFromDailyRow(parsed, cellValue(entry.row, 3));
    const resolvedOperator = resolveDailyReportOperator(sheetRows, entry.rowNum, entry.machine);
    ...
    const mainLines = applyProductInfoToLines(
      ...
      ...line,
      operator: resolvedOperator || line.operator || '',
    }));
