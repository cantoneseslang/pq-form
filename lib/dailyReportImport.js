...
import { normalizeSheetMonth, normalizeSheetYear, DEFAULT_DAILY_REPORT_MONTH, DEFAULT_DAILY_REPORT_YEAR } from './dailyReportSheetMap.js';
import {
  TYPE_ENGLISH_NAMES,
...
export function buildProductionRecordFromDailyRow({
  ...
  sheetMonth = DEFAULT_DAILY_REPORT_MONTH,
  sheetYear = DEFAULT_DAILY_REPORT_YEAR,
}) {
  ...
  return {
    ...
    dailyReport: {
      ...
      row: rowNum,
      machine: machine || '',
      month: String(sheetMonth ?? DEFAULT_DAILY_REPORT_MONTH),
      year: String(sheetYear ?? DEFAULT_DAILY_REPORT_YEAR),
    },
...
function resolveImportSheetMonth(options = {}) {
  return normalizeSheetMonth(options.month ?? options.sheetMonth) || DEFAULT_DAILY_REPORT_MONTH;
}

function resolveImportSheetYear(options = {}) {
  return normalizeSheetYear(options.year ?? options.sheetYear) || DEFAULT_DAILY_REPORT_YEAR;
}

function buildDailyReportLinkKey(sheetYear, sheetMonth, tabName, rowNum) {
  return `${sheetYear}:${sheetMonth}:${tabName}:${rowNum}`;
}

function linkKeyMatchesExisting(sheetYear, sheetMonth, tabName, rowNum, existingLinks) {
  const fullKey = buildDailyReportLinkKey(sheetYear, sheetMonth, tabName, rowNum);
  if (existingLinks.has(fullKey)) return true;
  if (sheetYear === DEFAULT_DAILY_REPORT_YEAR) {
    return existingLinks.has(`${sheetMonth}:${tabName}:${rowNum}`);
  }
  return false;
}

export async function scanDailyReportTab(tabName, options = {}) {
  const sheetMonth = resolveImportSheetMonth(options);
  const sheetYear = resolveImportSheetYear(options);
  const rows = await readRange(`${tabName}!A1:L150`, sheetMonth, sheetYear);
  ...
  return {
    ...
    sheetMonth,
    sheetYear,
    recordDateIso: dateInfo?.iso || null,
    dateInfo,
    entries,
    rows,
  };
...
export async function buildImportRecordsFromDailyTab(tabName, existingLinks = new Set(), options = {}) {
  const { recordDateIso, entries, rows, sheetMonth, sheetYear } = await scanDailyReportTab(tabName, options);
  const imported = [];
  const skipped = [];
  const errors = [];

  for (const entry of entries) {
    if (linkKeyMatchesExisting(sheetYear, sheetMonth, entry.tabName, entry.rowNum, existingLinks)) {
  ...
  return { tabName, sheetMonth, sheetYear, recordDateIso, imported, skipped, errors };
}

export function dailyReportLinkKey(dailyReport) {
  if (!dailyReport?.tabName || !dailyReport?.row) return '';
  const month = normalizeSheetMonth(dailyReport.month) || DEFAULT_DAILY_REPORT_MONTH;
  const year = normalizeSheetYear(dailyReport.year) || DEFAULT_DAILY_REPORT_YEAR;
  return buildDailyReportLinkKey(year, month, dailyReport.tabName, dailyReport.row);
}
...
export function collectExistingDailyReportLinks(records) {
  ...
  for (const record of records || []) {
    const dr = record?.dailyReport;
    if (!dr?.tabName || !dr?.row) continue;
    const month = normalizeSheetMonth(dr.month) || DEFAULT_DAILY_REPORT_MONTH;
    const year = normalizeSheetYear(dr.year) || DEFAULT_DAILY_REPORT_YEAR;
    links.add(buildDailyReportLinkKey(year, month, dr.tabName, dr.row));
    if (!dr.year) links.add(`${month}:${dr.tabName}:${dr.row}`);
  }
  return links;
}
...
export async function repairImportedProductNamesForTab(tabName, supabase, options = {}) {
  const { recordDateIso, entries, rows: dailySheetRows, sheetMonth, sheetYear } = await scanDailyReportTab(tabName, options);
  ...
  const dbRows = (allDbRows || []).filter((row) => {
    ...
    if (!dailyReport || String(dailyReport.tabName) !== String(tabName)) return false;
    const rowMonth = normalizeSheetMonth(dailyReport.month) || DEFAULT_DAILY_REPORT_MONTH;
    const rowYear = normalizeSheetYear(dailyReport.year) || DEFAULT_DAILY_REPORT_YEAR;
    return rowMonth === sheetMonth && rowYear === sheetYear;
  });
  ...
  if (!dbRows.length) {
    return {
      ...
      sheetMonth,
      sheetYear,
      recordDateIso: recordDateIso || null,
      repaired: [],
      skipped: [{ reason: 'no records for tab' }],
    };
  }
  ...
  return { tabName, sheetMonth, sheetYear, recordDateIso, repaired, skipped };
}
