...
export async function resolveProductFromDailyRow(parsed, length) {
  ...
  if (!typeKey) {
    const fallbackLabel = parsed.englishType || '';
    return {
      productNo: '',
      name: buildProvisionalProductName(fallbackLabel, spec),
      typeKey: '',
      ambiguous: false,
      candidates: [],
      plistMiss: true,
...
function applyProductInfoToLines(lines, productInfo) {
  return (lines || []).map((line) => {
    ...
    if (!currentName && nextName) copy.name = productInfo.name;
